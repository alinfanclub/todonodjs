<template>
  <div>
    ...
    <div>
      <ul>
        <li 
        v-for="(posts, index) in selectedPost" 
        :key="index"
        :item = "posts"
        :index = "index"
        class="postList"
...
export default {
  ...
  methods: {
    ...
    editPost(id, data, title, content, createAt, date, selectType) {
      ...
      this.$store.state.selectType = selectType,
      this.$router.replace("/edit/" + data)
    },
    nextPage () {
        this.pageNum += 1;
    },
  ...
  computed: {
    ...
    selectedPost () {
      return this.$store.state.postlist.filter((value) => value._id == this.$route.params.id)
    }
  }
}
</script>
